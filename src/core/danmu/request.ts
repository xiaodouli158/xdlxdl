...
function isElectronEnvironment() {
  ...
  // 综合判断
  return hasElectronProcess || hasElectronUA || hasElectronAPI;
}
...
export const fetchLiveInfo = async function (id: string) {
  try {
    ...
    console.log('fetchLiveInfo - isElectron:', isElectron, 'url:', url);
...
export const fetchImInfo = async function (roomId: string, uniqueId: string) {
  ...
  try {
    ...
    const baseUrl = isElectron
      ...
      : '/dylive';
    const url = `${baseUrl}/webcast/im/fetch/?${makeUrlParams(params)}`;

    console.log('fetchImInfo - isElectron:', isElectron, 'url:', url);
