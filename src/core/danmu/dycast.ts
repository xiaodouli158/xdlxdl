import { CLog } from '../../utils/logUtil.ts';
...
console.log('Environment detection:', {
  ...
  processType: typeof window !== 'undefined' && window.process ? window.process.type : 'none',
  hasElectronProcess: hasElectronProcess,
  hasElectronUA: hasElectronUA,
  hasElectronAPI: hasElectronAPI,
  isElectron: isElectron,
  userAgent: typeof navigator !== 'undefined' ? navigator.userAgent : 'none',
  locationOrigin: typeof location !== 'undefined' ? location.origin : 'none'
});

// 在浏览器环境中，使用当前页面的协议和主机，但确保使用正确的端口
let wsBaseUrl;
if (isElectron) {
  wsBaseUrl = 'wss://webcast5-ws-web-lf.douyin.com/webcast/im/push/v2/';
} else {
  // 浏览器环境：构建 WebSocket URL
  const protocol = location.protocol === 'https:' ? 'wss:' : 'ws:';
  const host = location.hostname;
  const port = location.port || (location.protocol === 'https:' ? '443' : '80');
  wsBaseUrl = `${protocol}//${host}:${port}/socket/webcast/im/push/v2/`;
}
